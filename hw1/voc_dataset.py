...
class VOCDataset(Dataset):
    ...
    def preload_anno(self):
        ...
        for index in self.index_list:
            ...
            root = tree.getroot()
            class_labels, weight_labels = [0] * 20, [1] * 20
            for obj in root.iter('object'):
                difficult = int(obj.find("difficult").text)
                class_name = obj.find("name").text
                class_labels[VOCDataset.get_class_index(class_name)] = 1
                if difficult == 1:
                    weight_labels[VOCDataset.get_class_index(class_name)] = 0
            
            label_list.append([class_labels, weight_labels])
    ...
    def __getitem__(self, index):
        ...
        if "train" in self.split:
            ...
            transform=transforms.Compose([
                transforms.Resize((int(1.05*self.size), int(1.05*self.size))),
