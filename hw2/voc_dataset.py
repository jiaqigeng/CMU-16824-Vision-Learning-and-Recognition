...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.roi_data['boxes'][0][index].shape[0] < self.top_n:
            boxes = self.roi_data['boxes'][0][index].astype(np.float)
        else:
            boxes = self.roi_data['boxes'][0][index][:self.top_n].astype(np.float)

        proposals = np.zeros(boxes.shape)
        proposals[:, 1] = boxes[:, 0] / height
        proposals[:, 0] = boxes[:, 1] / width
        proposals[:, 3] = boxes[:, 2] / height
        proposals[:, 2] = boxes[:, 3] / width
